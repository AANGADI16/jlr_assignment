...
#joining the two df
### by joining the condition 2 is satisifed
...
#Cleaning the transformed dataframe and then loading it into ADLS

df_Profit = df_main.drop(['Model_x','keybase','keyOptions','Option_Code','Model_y', 'nameplate_code'], axis = 1)
ProfitDF=spark.createDataFrame(df_Profit)
ProfitDF.coalesce(1).write.mode("overwrite").format("csv").option("header","true").csv("/mnt/jlrassig1/outputds")

# COMMAND ----------

#Loading the file from ADLS to check if the write operation executed correctly
...
df_g = df_grouped_OC.sort_values('sum', ascending = False)
df_g['x-labels'] = df_g['Options_Code'] +"-"+ df_g['Model_Text'].str[:4]
df_g
...
plt.xlabel('Options_Code and Model')
plt.ylabel('Total Profit £')
plt.title('Profit Per bottom 20 combinations')
plt.show()
...
df_Profit[(df_Profit['Model_Text'].str[:4] == 'L494') & (df_Profit['Options_Code'] == '129AH')  ]
